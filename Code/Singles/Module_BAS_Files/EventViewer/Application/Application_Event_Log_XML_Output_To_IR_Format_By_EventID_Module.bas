...
Sub Application_Event_Log_XML_Output_To_IR_Format_By_EventID()
...
MsgBox "Select the file that contains the Application EventIDs to Keep"

evtIdFilePath = Application.GetOpenFilename(Title:="Open Application EventIDs File")
...
'------------------------------------------------------------------------------
'Create an array with contents of Application EventIDs file
...
For n = 2 To lastRow
    Cells(n, "H") = "Evt ID: " & Cells(n, "H") & " | Evt Record #: " & Cells(n, "I")
Next
...
If Not hexData = 1005 Then
    ...
    For e = 1 To lastRow
        If IsEmpty(ActiveSheet.Cells(e, eventBin).Value) Then
            ActiveSheet.Cells(e, eventBin).Value = "-"
        End If
    Next
...
For f = 2 To lastRow3
    ...
    Cells(f, "F") = Cells(f, "F") & " |  " & Cells(f, "G")
    Cells(f, "I").Value = "Application Event Log"
Next

'------------------------------------------------------------------------------
'Delete Column
'------------------------------------------------------------------------------
Sheets(1).Columns("G").EntireColumn.Delete
